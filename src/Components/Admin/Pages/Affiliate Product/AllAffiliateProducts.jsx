    ...
    <div className="container">
      <div className="content-area">
        ...
        <div className="flex justify-between mb-4">
          ...
          />
          <button 
            onClick={() => navigate('/admin/products/add-affiliate/AddAffiliateProduct')}
            className="btn btn-primary rounded-2xl px-4 py-1 bg-violet-600 text-white hover:bg-violet-700 focus:outline-none">
      ...
      <th className="border border-gray-300 px-4 py-2">Gallery</th>
          ...
          <tbody>
            {currentProducts.map((product) => (
              <tr key={product._id} className="hover:bg-gray-100">
                <td className="border border-gray-300 px-4 py-2">{product.productName || 'N/A'}</td>
                        {/* Display main image */}
                        <td className="border border-gray-300 px-4 py-2">
  {product.featureImageSource === 'file' && product.featureImage ? (
    <img
      src={product.featureImage}
      alt={product.productName}
      className="w-16 h-16 object-cover rounded"
    />
  ) : product.featureImageSource === 'link' && product.featureImageLink ? (
    <a
      href={product.featureImageLink}
      target="_blank"
    ...
    'No Image or Link'
          ...
          {product.galleryImages?.length > 0 ? (
            <div className="flex space-x-1">
              ...
              {product.galleryImages.length > 3 && (
                <span className="text-gray-500 text-sm">+{product.galleryImages.length - 3}</span>
              )}
            </div>
          ) : (
            'No Gallery Images'
        ...
        {/* Display product link */}
                ...
                <td className="border border-gray-300 px-4 py-2 flex justify-center space-x-2">
                  <button
                    onClick={() => navigate(`/admin/products/edit-affiliate/${product._id}`)}
                    className="flex items-center rounded-2xl text-white bg-blue-900 hover:bg-blue-700 px-3 py-1 focus:outline-none focus:ring-2 transition ease-in-out duration-200">
                    <EditOutlined className="h-5 w-5 mr-1" />
                    Edit
                  </button>
