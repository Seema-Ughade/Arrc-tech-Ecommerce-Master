...
const ChildCategories = () => {
  ...
  const fetchChildCategories = async () => {
    try {
      ...
      if (Array.isArray(response.data)) {
        setChildCategories(response.data);
        toast.success('Child categories fetched successfully!');
      } else {
        console.error('Expected an array but got:', response.data);
        toast.error('Unexpected response format while fetching child categories.');
      }
    } catch (error) {
      console.error('Error fetching child categories:', error);
      toast.error('Failed to fetch child categories. Please try again.');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        await axios.put(`https://ecommerce-panel-backend.onrender.com/api/childcategories/${currentSubCategoryId}`, childCategoryData);
        toast.success('Child category updated successfully!');
      } else {
        await axios.post('https://ecommerce-panel-backend.onrender.com/api/childcategories', childCategoryData);
      ...
      toast.error('Error saving child category. Please try again.');
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this child category?')) {
      try {
        ...
        console.error('Error deleting child category:', error);
        toast.error('Error deleting child category. Please try again.');
      }
    }
  };
  ...
  const handleAttributeSubmit = async (e) => {
    ...
    try {
      if (editingAttributeId) {
        await axios.put(`https://ecommerce-panel-backend.onrender.com/api/childcategories/${selectedCategoryId}/attributes/${editingAttributeId}`, newAttribute);
        toast.success('Attribute updated successfully!');
      } else {
        await axios.post(`https://ecommerce-panel-backend.onrender.com/api/childcategories/${selectedCategoryId}/attributes`, newAttribute);
        toast.success('Attribute added successfully!');
      }
      setChildCategories(prev =>
        prev.map(cat => {
          if (cat._id === selectedCategoryId) {
            const updatedAttributes = editingAttributeId
              ? cat.attributes.map(attr => attr._id === editingAttributeId ? newAttribute : attr)
              : [...cat.attributes, newAttribute];
            return { ...cat, attributes: updatedAttributes };
          }
          return cat;
        })
      );
      ...
      toast.error('Error saving attribute. Please try again.');
    }
  };
  ...
  const handleEditClick = (childCategory) => {
    setSelectedCategory(childCategory.category);
    setSelectedSubCategory(childCategory.subCategory);
    setChildCategoryName(childCategory.name);
    setChildCategorySlug(childCategory.slug);
    setCurrentSubCategoryId(childCategory._id);
    setEditMode(true);
  ...
  const handleManageClick = (childCategory) => {
    setSelectedCategoryId(childCategory._id);
    setAttributesList(childCategory.attributes || []);
  ...
  const handleDeleteAttribute = async (attributeId) => {
    if (window.confirm('Are you sure you want to delete this attribute?')) {
      try {
        await axios.delete(`https://ecommerce-panel-backend.onrender.com/api/childcategories/${selectedCategoryId}/attributes/${attributeId}`);
        setAttributesList(prev => prev.filter(attr => attr._id !== attributeId));
        toast.success('Attribute deleted successfully!');
      } catch (error) {
        console.error("Error deleting attribute:", error);
        toast.error('Error deleting attribute. Please try again.');
      }
    }
  };
  ...
  const totalPages = Math.ceil(filteredChildCategories.length / itemsPerPage);
  const currentChildCategories = filteredChildCategories.slice(
    (currentPage - 1) * itemsPerPage, 
    currentPage * itemsPerPage
  );
      ...
      <div className="content-area">
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full">
              <h2 className="text-xl font-bold mb-4">{editMode ? 'Edit' : 'Add New'} Child Category</h2>
              <form onSubmit={handleSubmit}>
                <div className="mb-4">
                  <label className="block text-gray-700">Category*</label>
                  <select
        ...
        {/* Modal for managing attributes */}
        {showAttributes && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white w-1/2 p-6 rounded-lg shadow-lg">
              <h5 className="font-semibold text-lg mb-4">Attributes</h5>
              <ul className="list-disc pl-5">
                {attributesList.map((attribute) => (
                  <li key={attribute._id} className="flex justify-between items-center py-2 border-b">
                    <div className="flex flex-col">
        ...
        {isAttributeModalOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-8 rounded shadow-md w-96">
              <h2 className="text-2xl font-bold mb-4">{editingAttributeId ? 'Edit' : 'Add New'} Attribute</h2>
              <form onSubmit={handleAttributeSubmit}>
                <div className="mb-4">
                  <label className="block text-sm font-medium">Name *</label>
                  <input
                ...
                <div className="flex justify-between">
                  ...
                  <button
                    ...
                    className="btn px-4 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 focus:outline-none"
                  >
                    {editingAttributeId ? 'Update' : 'Create'} Attribute
                  </button>
                </div>
              </form>
            </div>
        ...
        <div className="mt-6">
          <table className="min-w-full bg-white border border-gray-200">
            ...
            <tbody>
              {currentChildCategories.map((childCategory) => (
                <tr key={childCategory._id} className="hover:bg-gray-100 text-center">
                  <td className="border px-4 py-2">{childCategory.category}</td>
                  <td className="border px-4 py-2">{childCategory.subCategory}</td>
                  <td className="border px-4 py-2">{childCategory.name}</td>
