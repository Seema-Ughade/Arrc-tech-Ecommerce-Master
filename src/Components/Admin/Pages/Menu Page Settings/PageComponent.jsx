...
const PageComponent = () => {
    ...
    const handleDescriptionChange = (content) => {
        setNewPage((prevPage) => ({ ...prevPage, description: content }));
    };
    ...
    const handleHeaderChange = async (HeaderId, newHeader) => {
        try {
            const response = await axios.put(
                `https://ecommerce-panel-backend.onrender.com/api/pages/${HeaderId}/Header`, 
                { Header: newHeader }
            );
            setPages(prev =>
                prev.map(page => (page._id === HeaderId ? { ...page, Header: newHeader } : page))
    ...
    const handleFooterChange = async (FooterId, newFooter) => {
        try {
            const response = await axios.put(
                `https://ecommerce-panel-backend.onrender.com/api/pages/${FooterId}/Footer`, 
                { Footer: newFooter }
            );
            setPages(prev =>
                prev.map(page => (page._id === FooterId ? { ...page, Footer: newFooter } : page))
                ...
                {isModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                        <div className="bg-white rounded-lg shadow-lg p-6 max-w-4xl w-full">
                            ...
                            <form onSubmit={editingPageId ? handleUpdatePage : handleSubmit}>
                                <div className="mb-4">
