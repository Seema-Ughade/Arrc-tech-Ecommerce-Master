...
// // import { TrashIcon } from '@heroicons/react/24/solid';
// // import { EditOutlined } from '@ant-design/icons';
// // import { fetchFonts, addFont, editFont, deleteFont } from '../../../../redux/actions/fontActions'; // Import the actions
// // import { filterFonts, setCurrentPage } from '../../../../redux/reducers/fontsReducer'; // Correct import from fontSlice

// // const Fonts = () => {
// //   const dispatch = useDispatch();
// //   const { fonts, filteredFonts, loading, error, message, currentPage } = useSelector((state) => state.fonts); // Access the Redux state
...
// //   useEffect(() => {
// //     dispatch(fetchFonts()); // Fetch fonts on component mount
// //   }, [dispatch]);

// //   useEffect(() => {
// //     dispatch(filterFonts(searchTerm)); // Filter fonts based on search term
// //   }, [searchTerm, dispatch]);
...
// //     e.preventDefault();
// //     dispatch(addFont(newFont)); // Dispatch addFont action
// //     setNewFont({ family: '' });
// //     setIsOpen(false);
// //   };

// //   const handleDelete = (id) => {
// //     dispatch(deleteFont(id)); // Dispatch deleteFont action
// //   };
...
// //     e.preventDefault();
// //     dispatch(editFont(editFont._id, editFont)); // Dispatch editFont action
// //     setIsEditOpen(false);
// //   };
...
//   const [newFont, setNewFont] = useState({ family: '' });
//   const [searchTerm, setSearchTerm] = useState('');
//   const [editFontData, setEditFontData] = useState(null); // Rename to avoid conflict with the action
//   const [isEditOpen, setIsEditOpen] = useState(false);
//   const itemsPerPage = 10;
...
//   const openEditModal = (font) => {
//     setEditFontData({ ...font }); // Open edit modal and set font data
//     setIsEditOpen(true);
//   };

//   const handleEditChange = (e) => {
//     const { name, value } = e.target;
//     setEditFontData({ ...editFontData, [name]: value });
//   };

//   const handleEditSubmit = (e) => {
//     e.preventDefault();
//     if (editFontData && editFontData._id) {
//       dispatch(editFont(editFontData._id, editFontData)); // Dispatch editFont action
//       setIsEditOpen(false); // Close modal after edit
//     }
//   };
...
const Fonts = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await dispatch(addFont(newFont)); // Dispatch addFont action
    setNewFont({ family: '' });
    setIsOpen(false);
    dispatch(fetchFonts()); // Refresh font list
  };

  const handleDelete = async (id) => {
    await dispatch(deleteFont(id)); // Dispatch deleteFont action
    dispatch(fetchFonts()); // Refresh font list
  };
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    if (editFontData && editFontData._id) {
      await dispatch(editFont(editFontData._id, editFontData)); // Dispatch editFont action
      setIsEditOpen(false); // Close modal after edit
      dispatch(fetchFonts()); // Refresh font list
    }
  };
