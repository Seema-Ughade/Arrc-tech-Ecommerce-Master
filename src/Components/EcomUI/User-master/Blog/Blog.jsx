  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const categorySet = new Set(blogData.map(blog => blog.category));
        const categoryData = Array.from(categorySet).map(category => ({
          name: category,
          count: blogData.filter(blog => blog.category === category).length
        }));
        categoryData.unshift({ name: "All", count: blogData.length });
        setCategories(categoryData);

        // Extract tags from blog data (assuming tags are available in the API response)
        const tagSet = new Set(blogData.flatMap(blog => blog.tags || []));
    ...
    <div>
      ...
      <div className="p-4 bg-white shadow-md">
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div>
              ...
              <div className="space-y-3 sm:space-y-4">
                {recentPosts.map((post) => (
                  ...
                  >
                    <img
                      src={post.featureImage || "/placeholder.svg?height=80&width=80"}
                      width={80}
                      height={80}
                      alt="Recent post"
          ...
          <div className="w-full lg:w-3/4 space-y-6">
            {currentBlogs.map((blog) => (
              <div key={blog._id} className="bg-white rounded-md shadow-md flex flex-col sm:flex-row">
                <img src={blog.featureImage || "/placeholder.svg?height=192&width=256"} alt={blog.title} className="h-48 w-full sm:w-1/3 object-cover" />
                <div className="p-4">
                  <h3 className="text-lg font-bold">{blog.title}</h3>
                  <p className="text-sm text-gray-600">{blog.excerpt}</p>
